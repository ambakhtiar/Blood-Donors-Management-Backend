---
name: problem-fix
description: "Problem Fix for Blood-Donors-Management-Backend. 9 error→fix pairs."
domain: problem-fix
composesFrom:
  - fix
triggers:
  - glob: "**/*"
autoGenerated: true
enabled: true
---

# Problem Fix

Auto-compiled from **3 real patterns** in **Blood-Donors-Management-Backend**. This skill is auto-routed to agents when working on problem-fix files.

## 🔧 Problem Playbooks

### Known Error → Fix

| Error Pattern | Fix | Seen |
|-------|-----|------|
| `PrismaClientKnownRequestError,` | problem-fix in edge.js | 5x |
| `"use strict";var h=Object.defineProperty;var T=Obj` | problem-fix in query_compiler_fast_bg.js | 3x |
| `import AppError from '../../errors/AppError';` | problem-fix in organisation.service.ts | 2x |
| `-     throw new AppError(httpStatus.NOT_FOUND, 'Ta` | Fixed null crash in AppError — externalizes configuration fo | 2x |
| `- import AppError from '../../errors/AppError';` | Fixed null crash in Secret — uses a proper password hashing  | 2x |
| `-     throw new AppError(httpStatus.FORBIDDEN, `Yo` | Fixed null crash in AppError — ensures atomic multi-step dat | 2x |
| `-             // throw new Error(`Environment vari` | Fixed null crash in EnvConfig — externalizes configuration f | 2x |
| `-         throw new AppError(` | Fixed null crash in Scenario — prevents null/undefined runti | 2x |
| `-             throw new AppError(status.INTERNAL_S` | Fixed null crash in EnvConfig — externalizes configuration f | 2x |

## 📐 Conventions & Best Practices

### Compiled Rules
- ✅ When encountering this, fix by: problem-fix in query_compiler_fast_bg.js (seen 2x)
- ✅ When encountering this, fix by: problem-fix in edge.js (seen 2x)
- ✅ When encountering this, fix by: problem-fix in index.js (seen 2x)

---
*Auto-generated by BrainSync 🧠 | 3 patterns | 2026-03-28*
